...
from .exceptions import FeatureNotFoundError, NamedFeatureError
...
def old_indices_from_layout(
    ...
    if not targets:
        raise FeatureNotFoundError(
            "Targets list cannot be empty when deriving indices."
        )
    ...
    if node_type_name is not None:
        node_slice = layout.slc(node_type_name)
        if node_slice.stop != feature_dim:
            raise NamedFeatureError(
                f"Node type feature '{node_type_name}' must be the last block in the layout "
                "to derive contiguous legacy indices."
            )
        indices["feature_index_end"] = node_slice.start
        indices["node_type_index"] = node_slice.start
    return indices
...
class LegacyIndexAdapter:
    ...
    def as_dict(self) -> Dict[str, int]:
        ...
        return old_indices_from_layout(self._layout, self._targets, self._node_type)
