...
class XFeatureLayout:
    ...
    def __init__(self, blocks: Iterable[Tuple[str, int]]):
        ...
        for name, size in blocks:
            ...
            if int(size) != size or size <= 0:
                raise ValueError(f"Size for '{name}' must be a positive integer, got {size!r}.")
            size = int(size)
    ...
    @staticmethod
    def from_dict(sizes: Mapping[str, int], order: Optional[Sequence[str]] = None) -> "XFeatureLayout":
