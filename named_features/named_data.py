...
from .exceptions import FeatureAssignmentError, FeatureNotFoundError, LayoutValidationError
...
class NamedData(Data):
    ...
    def __init__(
        self,
        *args,
        x_layout: XFeatureLayout,
        x_coords: Optional[Mapping[str, object]] = None,
        **kwargs,
    ) -> None:
        super().__init__(*args, **kwargs)
        if getattr(self, "x", None) is None:
            raise LayoutValidationError("NamedData requires an 'x' tensor to be present.")
    ...
    # Assignment
    # ------------------------------------------------------------------
    def _check_assignment_tensor(self, name: str, target: torch.Tensor, value: torch.Tensor) -> None:
    ...
    def x_assign(self, mapping: Mapping[str, torch.Tensor], *, inplace: bool = True) -> "NamedData":
    ...
    def x_reorder(self, new_order: Sequence[str], *, inplace: bool = True) -> "NamedData":
        """Reorder features according to ``new_order``."""

        new_layout = self.x_layout.reorder(new_order)
        reordered = torch.cat([self.x[..., self.x_slice(name)] for name in new_order], dim=-1)
        if inplace:
            self.x = reordered
    ...
    def x_drop(self, names: Sequence[str]) -> torch.Tensor:
        ...
        if not keep:
            raise FeatureNotFoundError("Dropping all features results in an empty selection.")
    ...
    # ------------------------------------------------------------------
    def x_to_dict(self, names: Optional[Sequence[str]] = None) -> Dict[str, torch.Tensor]:
    ...
    @staticmethod
    def x_from_dict(features: Mapping[str, torch.Tensor], order: Sequence[str]) -> torch.Tensor:
        """Pack features from a dictionary according to ``order``."""

        tensors = []
        for name in order:
            if name not in features:
                raise FeatureNotFoundError(f"Feature '{name}' missing from provided mapping.")
    ...
    def clone(self) -> "NamedData":  # pragma: no cover - relies on Data.clone
        out = super().clone()
        out.x_layout = self.x_layout
        out.x_coords = dict(self.x_coords)
        return out

    def __repr__(self) -> str:  # pragma: no cover - debug helper
        return f"NamedData({super().__repr__()}, x_layout={self.x_layout.sizes()})"
