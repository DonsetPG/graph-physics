...
class DatasetBase(optimizers.Objective): 
    ...
    def initialize_parameters(self, theta):  # This method is used to initialize the parameters variable
        # Keep optimizer parameters on the same device/dtype as the dataset tensors.
        # Otherwise, we can hit matmul dtype mismatches (e.g. float64 parameters from
        # NumPy initialization vs float32 data from a PyTorch model).
        dtype = None
        try:
            gm = self.g_mean
