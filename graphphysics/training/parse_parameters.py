...
def _normalise_targets(
    ...
    if isinstance(direct_targets, Sequence) and not isinstance(direct_targets, (str, bytes)):
    ...
    dataset_targets = dataset_cfg.get("targets") if isinstance(dataset_cfg, Mapping) else None
    if isinstance(dataset_targets, Sequence) and not isinstance(dataset_targets, (str, bytes)):
...
def _derive_placeholder_layout(index_cfg: Mapping[str, Any]) -> tuple[XFeatureLayout, Optional[str]]:
    start = int(index_cfg.get("feature_index_start", 0))
    end = index_cfg.get("feature_index_end")
    if end is None:
        raise ValueError(
            "Legacy configuration missing 'feature_index_end'; cannot derive placeholder layout."
        )
    end = int(end)
    if end <= start:
        raise ValueError(
            "Legacy configuration has non-positive feature dimension; check 'feature_index_start'/'feature_index_end'."
        )

    names = [f"legacy_feature_{i:03d}" for i in range(end - start)]
    sizes = {name: 1 for name in names}
    layout = make_x_layout(names, sizes)

    node_type_index = index_cfg.get("node_type_index")
    node_type_name: Optional[str] = None
    if node_type_index is not None:
        node_type_index = int(node_type_index)
        if start <= node_type_index < end:
            node_type_name = names[node_type_index - start]

    return layout, node_type_name
...
def _resolve_mode(
    named_section: Dict[str, Any], mode_override: Optional[str]
) -> str:
    if mode_override:
...
def prepare_parameters(
    ...
    *,
    ...
    if isinstance(features_cfg, Mapping) and "node" in features_cfg:
        ...
        if not isinstance(node_order, Sequence) or isinstance(node_order, (str, bytes)) or not node_order:
            raise ValueError(
        ...
        if isinstance(legacy_index_cfg, Mapping) and mode != "legacy":
            ...
            if mismatches:
                ...
                logger.warning(
                    "Legacy index configuration disagrees with derived layout values (%s); using derived indices.",
                    mismatch_strings,
                )
    ...
    if isinstance(index_cfg, Mapping) and "x_layout" not in named_section:
        layout, node_type_name = _derive_placeholder_layout(index_cfg)
        named_section.setdefault("x_layout", layout)
        named_section.setdefault("node_features", layout.names())
        named_section.setdefault("sizes", layout.sizes())
        named_section.setdefault("node_type", node_type_name)
        targets = _normalise_targets(param, dataset_cfg)
        if targets:
...
def get_preprocessing(
    ...
    if world_pos_params.get("use", False):
        if "world_pos_feature" in world_pos_params or "displacement_feature" in world_pos_params:
...
def get_simulator(param: Dict[str, Any], model, device: torch.device) -> Simulator:
    ...
    targets = named_section.get("targets") if isinstance(named_section, Mapping) else None
...
def get_dataset(
    ...
    x_coords = named_section.get("x_coords") if isinstance(named_section, Mapping) else None
