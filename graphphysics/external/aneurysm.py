...
def _maybe_named_feature(graph: Data, candidates: Sequence[str], index: int) -> torch.Tensor:
...
def aneurysm_node_type(graph: Data) -> torch.Tensor:
    ...
    node_type = torch.zeros(velocity.shape[0], device=velocity.device, dtype=velocity.dtype)
