...
def get_frame_as_mesh(
    ...
    *,
    ...
    mesh_pos = mesh_pos_source[frame] if mesh_pos_source.ndim > 1 else mesh_pos_source
    num_nodes = mesh_pos.shape[0]
    cells_source = traj["cells"]
    cells = cells_source[frame] if cells_source.ndim > 1 else cells_source

    dynamic_keys = [
        key for key in traj.keys() if key not in {"mesh_pos", "cells", "node_type"}
    ]
    ...
    processed_dynamic = {
        key: _normalise_feature_array(_extract_frame(traj[key], frame))
    ...
    if layout_names:
        for name in layout_names:
            if name == "node_type":
                ...
                ordered_point_data[name] = _normalise_feature_array(mesh_pos)
        ...
        for name in processed_dynamic.keys():
