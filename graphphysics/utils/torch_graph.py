...
def compute_k_hop_graph(
    ...
    if isinstance(graph, NamedData):
        ...
        khop_mesh_graph.edge_index = khop_edge_index
    ...
    *,
    ...
    if target is not None and len(target) > 0:
        if feature_order is None:
            stacked_target_names = list(target.keys())
        else:
            stacked_target_names = [name for name in feature_order if name in target]
        target_names = tuple(stacked_target_names)
    ...
    if x_layout is not None:
        data = NamedData(
            x=node_features, x_layout=x_layout, x_coords=x_coords, **data_kwargs
        )
    else:
        data = Data(x=node_features, **data_kwargs)

    if target_features is not None and target_names:
...
def torch_graph_to_mesh(
    graph: Data,
    node_features_mapping: Optional[Union[Mapping[str, int], Sequence[str]]] = None,
) -> Mesh:
