...
class MLP(torch.nn.Module):
    def __init__(
        self, input_dim, latent_dim, output_dim, hidden_layers, layer_normalized=True
    ):
...
class GMP(torch.nn.Module):
    ...
    def forward(self, x, g, pos):
        ...
        dir = pi - pj  # (N, pos_dim)
        norm = torch.norm(dir, dim=-1, keepdim=True)  # (N, 1)
        fiber = torch.cat([dir, norm], dim=-1)  # (N, pos_dim+1)
...
class WeightedEdgeConv(torch.nn.Module):
    ...
    def cal_ew(self, w, g):
        ...
        -------
        tuple
            Edge weights for convolution and aggregated node weights
            (used for iteratively calculating this in the next layer).
        """
