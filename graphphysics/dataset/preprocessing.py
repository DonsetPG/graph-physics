...
def _feature_name_from_index(
    layout: Optional["XFeatureLayout"], index: int
) -> Optional[str]:
    if layout is None:
        return None
    for block in layout.blocks:
...
def _translate_world_params(
    ...
    if params is None:
        return None
    result = dict(params)
    provided_world_name = "world_pos_feature" in result
    if layout is None:
        return result
    ...
    if world_name and (
        ...
        if name:
            ...
            if not provided_world_name:
                result["_world_pos_feature_inferred"] = True
...
def _translate_noise_params(
    ...
    if features is not None and (
        ...
        result.setdefault("noise_index_start", starts if len(starts) > 1 else starts[0])
...
class _OptionalFaceToEdge(BaseTransform):
    """Safely convert faces to edges when present on the graph."""

    def __init__(self, *, remove_faces: bool = False) -> None:
        super().__init__()
        self._transform = T.FaceToEdge(remove_faces=remove_faces)
    ...
    *,
    ...
    if use_named:
        ...
        if getattr(graph, "y", None) is None:
            raise ValueError(
                "add_obstacles_next_pos requires 'graph.y' to compute displacements."
            )

        target_names = tuple(getattr(graph, "target_names", ()))
        if target_names:
            sizes = layout.sizes()
            offset = 0
            ...
            for name in target_names:
                if name not in sizes:
                    raise ValueError(
                        f"Target feature '{name}' not present in x-layout; cannot align targets."
                    )
                size = sizes[name]
                if name == target_name:
                    if offset + size > graph.y.size(-1):
                        raise ValueError(
                            f"Target tensor for feature '{target_name}' is truncated; "
                            f"expected at least {offset + size} columns, got {graph.y.size(-1)}."
                        )
                    target_world_pos = graph.y[:, offset : offset + size]
                    break
                offset += size
            if target_world_pos is None:
                raise ValueError(
                    f"Target feature '{target_name}' missing from graph.target_names {target_names}."
                )
        else:
            target_slice = layout.slc(target_name)
            if target_slice.stop > graph.y.size(-1):
                raise ValueError(
                    f"Target tensor does not contain feature '{target_name}' "
                    "according to the x-layout slices. "
                    "Ensure the dataset exposes 'graph.target_names' metadata."
                )
            target_world_pos = graph.y[:, target_slice]
    ...
    *,
    ...
    if use_named:
        if noise_features is None:
            raise ValueError("Named usage of add_noise requires 'noise_features'.")
    ...
    if node_type_index is None:
        raise ValueError("Legacy usage of add_noise requires 'node_type_index'.")
...
def build_preprocessing(
    ...
    *,
    ...
    if world_pos_parameters is not None:
        inferred_world_name = world_pos_parameters.pop(
            "_world_pos_feature_inferred", False
        )
        if (
            "world_pos_feature" in world_pos_parameters
            and "displacement_feature" not in world_pos_parameters
        ):
            if inferred_world_name:
                # Fall back to legacy behaviour when the name was inferred without explicit displacement.
                world_pos_parameters.pop("world_pos_feature", None)
            else:
                raise ValueError(
        ...
        preprocessing.extend(
            [
                partial(
                    ...
                    **(
                        {
                            "world_pos_feature": world_pos_parameters[
                                "world_pos_feature"
                            ],
                ...
                _OptionalFaceToEdge(remove_faces=False),
                partial(
                    add_world_edges,
                    **(
                        {
        ...
        preprocessing.append(_OptionalFaceToEdge(remove_faces=False))
    ...
    if noise_parameters is not None:
        ...
        if (
            "noise_features" in noise_parameters
            and not noise_parameters["noise_features"]
        ):
            raise ValueError(
                "Named noise configuration requires non-empty 'noise_features'."
            )
