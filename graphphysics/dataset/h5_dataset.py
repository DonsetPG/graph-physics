...
class H5Dataset(BaseDataset):
    ...
    def _build_node_features(self, traj: dict, frame: int) -> Union[torch.Tensor, Dict[str, np.ndarray]]:
        ...
        if self.x_layout is None:
            ...
            time_column = np.full((node_features.shape[0], 1), time_value, dtype=np.float32)
