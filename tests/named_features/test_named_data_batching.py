...
def make_batch_samples():
    ...
    for nodes in [3, 5]:
        ...
        samples.append(NamedData(x=x, edge_index=torch.empty((2, 0), dtype=torch.long), x_layout=layout))
    return samples
...
def test_batching_preserves_layout():
    ...
    assert batch.x_sel("a").shape[0] == sum(sample.num_nodes for sample in make_batch_samples())
