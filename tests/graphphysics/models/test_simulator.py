...
class MockModel(nn.Module):
    ...
    def forward(self, input_graph: Data) -> torch.Tensor:  # type: ignore[override]
        return torch.zeros(input_graph.x.shape[0], self.output_size, device=input_graph.x.device)
...
def test_selectors_use_named_layout(simulator: Simulator, sample_data: NamedData) -> None:
    selected = simulator.select_targets_from_x(sample_data)
    torch.testing.assert_close(selected, sample_data.x_sel(["velocity_x", "velocity_y"]))


def test_assignment_updates_named_features(simulator: Simulator, sample_data: NamedData) -> None:
    new_values = torch.ones(sample_data.num_nodes, 2)
    simulator.assign_targets_to_x(sample_data, new_values)
    torch.testing.assert_close(sample_data.x_sel(["velocity_x", "velocity_y"]), new_values)
...
def test_one_hot_type_uses_named_lookup(simulator: Simulator, sample_data: NamedData) -> None:
    node_type = simulator.get_node_type(sample_data)
    assert node_type.shape == (sample_data.num_nodes,)
    assert (node_type >= 0).all()


def test_build_outputs_preserves_named_targets(simulator: Simulator, sample_data: NamedData) -> None:
    simulator.eval()
